...
exports.registerUser = (req, res) => {
  const { name, email, password, address, key } = req.body;

  const result = registerSchema.validate({ name, email, password, address, key });
  if (!result.error) {
  ...
  if (!result.error) {
    ...
    User.findOne({ email }).then((user) => {
      ...
      bcrypt.compare(password, user.password).then((isMatch) => {
        ...
        const sessUser = { id: user.id, name: user.name, email: user.email, balance: user.balance, release1: user.release1, release2: user.release2, address: user.address,
          key: user.keys, };
