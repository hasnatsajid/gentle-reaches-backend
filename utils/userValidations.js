...
const name = Joi.string().min(5).max(90).pattern(/^[a-zA-Z0-9' ]{3,20}$/).required().messages({
...
const address = Joi.string().optional();
const key = Joi.string().optional();

exports.loginSchema = Joi.object({ email, password});

exports.registerSchema = Joi.object({ name, email, password, address, key});